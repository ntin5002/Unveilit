...
import { db } from "@/db";
import { productAuditRecords, externalResourceLinks } from "@/db/schema";
...
function verifySignature(rawBody: string, provided: string | null) {
  ...
  if (!secret) {
    throw new HttpError(503, "SIGNATIVE_WEBHOOK_NOT_CONFIGURED", "Signative integration webhook secret is not configured.");
  }
  if (!provided) throw new HttpError(401, "INVALID_SIGNATURE", "Missing Signative signature.");
  ...
  if (a.length !== b.length || !timingSafeEqual(a, b)) {
    throw new HttpError(401, "INVALID_SIGNATURE", "Invalid Signative signature.");
  }
}
