...
async function authorizeLocalResource(
  ...
  switch (localResourceType) {
    ...
    case "CONTACT":
      ...
      break;
    case "ORDER":
      assertCapability(context, manage ? PhotoCapabilities.paymentsManage : PhotoCapabilities.paymentsView);
      await getOrderForContext(context, localResourceId);
      break;
    default:
      throw new HttpError(400, "UNSUPPORTED_LOCAL_RESOURCE", "Supported Signative link targets are GALLERY, CONTACT, and ORDER.");
  }
  return context;
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ success: true, data, message: "Signative resource linked successfully" }, { status: 201 });
  } catch (error) {
    return apiError(error);
  }
}
