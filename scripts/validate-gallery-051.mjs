...
const settings = read("src/features/capture-protection/ProtectionSettingsPanel.tsx");
const livePreview = read("src/features/capture-protection/WatermarkSettingsPreview.tsx");
...
check("live theft-prevention image preview", settings.includes("WatermarkSettingsPreview") && livePreview.includes("Live protection preview") && livePreview.includes('imageUrl || "/demo/photo-1.svg"'));
check("live preview tracks watermark controls", livePreview.includes("customText") && livePreview.includes("opacity") && livePreview.includes("density") && livePreview.includes("dynamicOverlay") && livePreview.includes("proofLongEdge"));
check("gallery proof image feeds live preview", detail.includes("protectionPreviewImageUrl") && detail.includes("previewImageUrl={protectionPreviewImageUrl}"));
check("gallery notifications are top-right above modal blur", detail.includes("gallery-toast") && css.includes("top: 24px") && css.includes("z-index: 2147482500"));
check("share copy notification reflects clipboard result", detail.includes("let copied = false") && detail.includes("Use the Copy button to copy it"));
check("diagnostic curtain force-restores", protectedGallery.includes('scheduleRestore(delay, source === "simulation")') && protectedGallery.includes("diagnostic-auto-restore"));
check("diagnostic attempts isolated from viewer attempts", protectedGallery.includes("photo-protection-diagnostic-attempt-count") && protectedGallery.includes("attemptStorageKey"));
check("diagnostic curtain has direct reset escape", protectedGallery.includes("Reset / dismiss demo curtain") && protectedGallery.includes("resetDiagnosticProtection"));

const failures = checks.filter((item) => !item.pass);
console.log(`\n${checks.length - failures.length}/${checks.length} gallery 0.5.1 checks passed.`);
